...
type FilterFormProps = {
  ...
  onChange: (key: keyof FilterValues, subKey: "min" | "max", value: string) => void
}
        ...
        <div key={key} className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Min"
              value={value.min}
              onChange={(e) => onChange(key as keyof FilterValues, "min", e.target.value)}
              className="flex-1"
            />
            <Input
              id={`${key}-max`}
              placeholder="Max"
              value={value.max}
              onChange={(e) => onChange(key as keyof FilterValues, "max", e.target.value)}
              className="flex-1"
            />
          </div>
        </div>
